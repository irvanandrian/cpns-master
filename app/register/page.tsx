...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!paket) {
      alert("Silakan pilih kategori ujian terlebih dahulu!");
      return;
    }
    ...
    try {
      ...
      if (authData.user) {
        // Menyimpan kategori ke kolom 'paket' di tabel profiles
        const { error: profileError } = await supabase
          ...
          .insert([{ 
            ...
            whatsapp, 
            email,
            paket // Isinya string kategori (ex: "cpns")
          }]);
        
        if (profileError) throw profileError;
        
        alert("Pendaftaran Berhasil! Akses kategori " + paket.toUpperCase() + " telah diaktifkan.");
        router.push('/login');
      }
    } catch (error: any) {
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#FDFBF9] p-4 font-sans text-[#42271E]">
      <div className="bg-white p-10 rounded-[2.5rem] shadow-[0_20px_50px_rgba(93,64,55,0.1)] w-full max-w-md border border-[#E6CEA0]/30 relative overflow-hidden">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[#A67C52] font-semibold mt-1 text-xs tracking-widest uppercase italic">"Satu Akses Untuk Semua Paket"</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4 relative z-10">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="email" placeholder="email@com" required className="w-full px-5 py-3.5 rounded-2xl border border-[#E6CEA0] outline-none focus:ring-2 focus:ring-[#A67C52] bg-[#FDFBF9]" onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-black text-[#5D4037] uppercase tracking-widest mb-1.5 ml-1">WhatsApp</label>
              <input type="text" placeholder="08xxx" required className="w-full px-5 py-3.5 rounded-2xl border border-[#E6CEA0] outline-none focus:ring-2 focus:ring-[#A67C52] bg-[#FDFBF9]" onChange={(e) => setWhatsapp(e.target.value)} />
            </div>
          </div>

          {/* PILIHAN KATEGORI (SIMPEL) */}
          <div>
            <label className="block text-xs font-black text-[#5D4037] uppercase tracking-widest mb-1.5 ml-1">Pilih Kategori Ujian</label>
            <select 
              required 
              value={paket}
              onChange={(e) => setPaket(e.target.value)}
              className="w-full px-5 py-3.5 rounded-2xl border-2 border-[#E6CEA0] outline-none focus:ring-2 focus:ring-[#5D4037] bg-white font-bold text-sm text-[#5D4037] cursor-pointer"
            >
              <option value="" disabled>-- Pilih Kategori --</option>
              <option value="cpns">CPNS (Semua Paket)</option>
        ...
        <div className="mt-10 pt-6 border-t border-[#E6CEA0]/30 text-center">
          <p className="text-sm text-[#795548] font-medium italic">Sudah punya akun? <Link href="/login" className="text-[#A67C52] font-black hover:underline uppercase tracking-tighter">Login</Link></p>
        </div>
      </div>
    </div>
  );
