...
export default function Home() {
  ...
  const [statusSoal, setStatusSoal] = useState({
    ...
    snbt: false,
    kedinasan: false,
    psikotes: true // Psikotes selalu tersedia
  });
  ...
  const paketHarga = [
    { id: 'cpns', title: "Paket CPNS", harga: "50k", fitur: ["10x Try Out Full", "Pembahasan Teks", "Grup Belajar", "Akses Selamanya"] },
    { id: 'psikotes', title: "Paket Psikotes", harga: "50k", fitur: ["Tes Koran (Pauli/Krae)", "Tes Kecermatan", "Grafik Statistik Kecepatan", "Latihan Tanpa Batas"] },
    { id: 'bumn', title: "Paket BUMN ", harga: "50k", fitur: ["Materi AKHLAK", "TKD Standard FHCI", "Grup Sharing", "Update Soal Berkala"] },
    { id: 'kedinasan', title: "Paket Sekolah Kedinasan", harga: "50k", fitur: ["Simulasi SKD Lengkap", "Bank Soal 5 Tahun", "Konsultasi Jurusan"] },
    { id: 'snbt', title: "Paket SNBT", harga: "30k", fitur: ["Trik Cepat PK", "Penalaran Umum"] }
  ];

  const testimoniData = [
    { text: "Soal TKP-nya sangat mirip asli!", user: "Andini, Lolos NIP" },
    { text: "Materi AKHLAK-nya daging banget!", user: "Rian, Staff BUMN" },
    { text: "Tes Kecermatannya bikin mata terlatih!", user: "Dedi, Casis Polri" },
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF9] text-[#42271E] font-sans select-none scroll-smooth relative">
      ...
      <section id="paket" className="py-24 px-6 max-w-[90rem] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          {paketHarga.map((item) => {
            ...
            return (
              <div key={item.id} className={`bg-[#5D4037] rounded-[3rem] p-8 shadow-2xl border-4 ${item.id === 'psikotes' ? 'border-[#E6CEA0]' : 'border-[#E6CEA0]/20'} flex flex-col relative overflow-hidden transition-all hover:scale-[1.03]`}>
                <div className={`absolute top-6 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${isReady ? 'bg-green-500 text-white' : 'bg-red-500 text-white animate-pulse'}`}>
                  {isReady ? '● Tersedia' : '○ Belum Tersedia'}
                </div>
