...
function UjianContent() {
  ...
  useEffect(() => {
    const fetchSoal = async () => {
      ...
      if (!error && data) {
        const urutan = { "TWK": 1, "TIU": 2, "TKP": 3, "TPA": 1, "TBI": 2 };
  ...
  const hitungSkor = () => {
    ...
    setJawabanUser((currentJawaban: any) => {
      soal.forEach((item) => {
        const pilihanUser = currentJawaban[item.id] || null;
        const kat = item.kategori || "UMUM";
        let poin = 0;

        if (!recapKategori[kat]) recapKategori[kat] = 0;

        const hasPointSystem = item.poin_a !== undefined && item.poin_a !== null;
          ...
          <div className="max-w-3xl mx-auto p-4 space-y-4">
            {dataReview.map((item, idx) => (
              <div key={idx} className="bg-white p-5 rounded-2xl border border-[#E6CEA0]/30 shadow-sm relative overflow-hidden">
                ...
                {item.image_url && <img src={item.image_url} alt="soal" className="max-h-40 mx-auto mb-4 rounded-lg shadow-sm border" />}
